...
export default function Payment() {
  ...
  const itemsInObj = checkout.checkoutToken.live?.line_items.reduce((result, item, index, array) => {
    result[item.id] = {
  ...
  function captureCheckout(b) {
    ...
    commerce.checkout.capture(checkoutId, data).then((response) => console.log(response));
  }
          ...
          {({ elements, stripe }) => (
            <form onSubmit={(e) => handleSubmit(e, elements, stripe)}>
              ...
              <div>
                ...
                <button>PAY: {cart.subtotal.raw} </button>
              </div>
            </form>
          )}
        </ElementsConsumer>
